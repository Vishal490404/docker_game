...
progress_bar() {
    ...
    printf "%*s\n" "$spaces" " "  # Add spaces for centering
}
...
if docker images --format '{{.Repository}}:{{.Tag}}' | grep -q "^wildwarrior44/alpine:93.56$"; then
    visited[0]=1
...
if docker images --format '{{.Repository}}:{{.Tag}}' | grep -q "^bash:latest$" && [ "${visited[0]}" -eq 1 ]; then 
    visited[1]=1
    # echo "again"
...
if docker images --format '{{.Repository}}:{{.Tag}}' | grep -q "^bash:56.93$" && [ "${visited[1]}" -eq 1 ]; then 
    visited[2]=1
    echo "Great!! Task-3 is completed."
    # progress_bar 60
else
...
if docker ps -a --format '{{.Names}}' | grep -q "^alphabetameta$" && [ "${visited[2]}" -eq 1 ]; then
    ...
    if [ "$container_image" == "$expected_image" ]; then
        visited[3]=1
        echo "Great!! Task-4 is completed, container creation completed successfully with the correct image."
        # progress_bar 80
...
if docker ps -a --format '{{.Names}}' | grep -q "^alphabetameta$" && [ "${visited[4]}" -eq 1 ]; then
...
# check if task-7 is completed
if docker images --format '{{.Repository}}:{{.Tag}}' | grep -q "^busybox:latest$" && [ "${visited[5]}" -eq 1 ]; then 
    echo "Incomplete task-7"
    # progress_bar 100
elif  [ "${visited[5]}" -eq 1 ]; then
    visited[6]=1
    echo "Great!! Task-7 is completed."
else
    echo "Incomplete Task-7"
fi
...
if [ "${visited[0]}" -eq 0 ]; then
    ...
    progress_bar 80
elif [ "${visited[5]}" -eq 1 ] && [ "${visited[6]}" -eq 0 ]; then
    echo "You are 90% there"
    progress_bar 90
elif [ "${visited[6]}" -eq 1 ]; then
    echo "You made it"
    progress_bar 100
fi

if [ "${visited[6]}" -eq 1 ]; then
    echo "Now that you have come so far, find the flag which is hidden in the text file."
    echo "flag{aqspd7443e}" > not_a_flag.txt
fi
